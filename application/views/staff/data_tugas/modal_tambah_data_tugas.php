<form method="POST" id="form_tambah" enctype="multipart/form-data">
    ...
    <input type="hidden" name="id_jadpel_tugas" value="<?= $where->id_jadpel ?>">
    ...
    <div class="modal-body">
        ...
        <div class="row">
            <div class="col mb-1">
                ...
                <input type="text" name="judul_tugas" class="form-control" placeholder="Masukan Judul Tugas" required>
            </div>

            <div class="col mb-1">
                <label for="nameLarge" class="form-label">Deskripsi Tugas</label>
                <textarea type="text" name="des_tugas" class="form-control" placeholder="Masukan Deskripsi Tugas" required></textarea>
            </div>
        </div>
        ...
        <div class="row">
         <div class="col mb-1">
            ...
            <input type="text" name="tgl_mulai_tugas" class="form-control tanggal" placeholder="Masukan Tanggal Mulai" required>
        </div>

        <div class="col mb-1">
            <label for="nameLarge" class="form-label">Tanggal Selesai</label>
            <input type="text" name="tgl_selesai_tugas" class="form-control tanggal" placeholder="Masukan Tanggal Selesai" required>
        </div>
    </div>
...
/*Proses Tambah Data Tugas*/
    $('#form_tambah').on('submit', function(e) {
        ...
        Swal.fire({
            ...
            if(result.value) {
                $.ajax({
                    url: "<?= site_url('Jadwal_mengajar/proses_tambah_data_tugas')?>",
                    method: "POST",
                    ...
                    success: function(data){
                        ...
                        if(data.status == true) {
                          Swal.fire({
                            ...
                            icon: "success",
                            title: "Berhasil",
                            text: "Data Tugas Berhasil ditambah!",
                            timer: 1500,
                        }).then((e)=> {
                            $('#table_data_tugas').DataTable().ajax.reload(null, false);
                            $('#modal_tambah_data_tugas').modal('hide');
                        });
                    } else {
                      Swal.fire({
                        icon: "error",
